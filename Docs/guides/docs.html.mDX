...
The example below demonstrates how to generate text with [Pipeline](/docs/transformers/main/en/main_classes/pipelines#transformers.Pipeline), [AutoModel](/docs/transformers/main/en/model_doc/auto#transformers.AutoModel), and from the command line using the instruction-tuned models.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/configuration_qwen2.py#L25)
...
defaults will yield a similar configuration to that of the [Qwen/Qwen2-7B-beta](https://huggingface.co/Qwen/Qwen2-7B-beta)
...
rms_norm_eps (`int`, *optional*, defaults to `1e-06`) : The epsilon used by the rms normalization layers.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/tokenization_qwen2.py#L36)

save_vocabularytransformers.Qwen2Tokenizer.save_vocabularyhttps://github.com/huggingface/transformers/blob/main/src/transformers/tokenization_utils_tokenizers.py#L487[{"name": "save_directory", "val": ": str"}, {"name": "filename_prefix", "val": ": str | None = None"}]
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L249)

forwardtransformers.Qwen2RMSNorm.forwardhttps://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L258[{"name": "hidden_states", "val": ": Tensor"}]

## Qwen2Model[[transformers.Qwen2Model]]

#### transformers.Qwen2Model[[transformers.Qwen2Model]]

[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L333)
...
elements depending on the configuration ([Qwen2Config](/docs/transformers/main/en/model_doc/qwen2#transformers.Qwen2Config)) and inputs.
The [Qwen2Model](/docs/transformers/main/en/model_doc/qwen2#transformers.Qwen2Model) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L418)
...
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration ([Qwen2Config](/docs/transformers/main/en/model_doc/qwen2#transformers.Qwen2Config)) and inputs.
The [Qwen2ForCausalLM](/docs/transformers/main/en/model_doc/qwen2#transformers.Qwen2ForCausalLM) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L491)
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/qwen2/modeling_qwen2.py#L495)
...
- **use_cache** (`bool`, *optional*) --
  If set to `True`, `past_key_values` key value states are returned and can be used to speed up decoding (see
  `past_key_values`).0[TokenClassifierOutput](/docs/transformers/main/en/main_classes/output#transformers.modeling_outputs.TokenClassifierOutput) or `tuple(torch.FloatTensor)`A [TokenClassifierOutput](/docs/transformers/main/en/main_classes/output#transformers.modeling_outputs.TokenClassifierOutput) or a tuple of
`torch.FloatTensor` (if `return_dict=False` is passed or when `config.return_dict=False`) comprising various
elements depending on the configuration (`None`) and inputs.
